...
This template includes one removable demo resource to show the project layering:

```http
GET /demo/users?email=ada@example.com
```

The endpoint reads from the `demo_user` table through `DemoUserRepository` and
returns a `DemoUserRead` payload inside the standard success response envelope.
Demo files, classes, paths, and table names use the `demo` prefix so they can be
removed cleanly when real domain resources replace them.
...
- SQLAlchemy MySQL connections use a pool controlled by `MYSQL_POOL_SIZE`.
  Readiness checks retry with `MYSQL_RETRY_ATTEMPTS` and
...
The image runs as a non-root user and includes a Docker healthcheck against
`/health/live`.
